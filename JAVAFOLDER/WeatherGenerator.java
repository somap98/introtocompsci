...
public class WeatherGenerator {
    ...
    for(int i = 0; i < 12; i++) {
        drywetProbability[i] = drywet[x][i+2];
        wetwetProbability[i] = wetwet[x][i+2];
    }
     ...
     *      return result will still be different for each run since it is randomly generated.z
     */
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        forecast = forecastGenerator(drywetProbability[month], wetwetProbability[month], numberOfDaysInMonth[month]);
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i = 1; i < forecast.length; i++) {
            ...
            if(newStreak > oldStreak && forecast[i] == mode) {
    ...
    public static void main (String[] args) {
        ...
        StdRandom.setSeed(1668203484033L);
        System.out.println(bestWeekToTravel(oneMonthForecast(4100, 9, -119.19, 45.76)));
